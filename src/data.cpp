...
Tensor from_flat_vector(const std::vector<T>& data, const std::vector<size_t>& shape, DType dtype, bool requires_grad ) {
    ...
    for (size_t i = 0; i < expected; ++i) {
        ...
        write_scalar_at(out.impl->data->data.get(), i, out.impl->dtype, static_cast<double>(data[i]));
    }
    return out;
}
// ---------- raw pointer -> tensor ----------
...
Tensor from_raw_ptr(const T* ptr, size_t count, const std::vector<size_t>& shape, DType dtype , bool requires_grad ) {
    ...
    Tensor out(shape, dtype, requires_grad);
    for (size_t i = 0; i < expected; ++i) {
        write_scalar_at(out.impl->data->data.get(), i, out.impl->dtype, static_cast<double>(ptr[i]));
    }
    return out;
}
...
Tensor from_2d_vector(const std::vector<std::vector<T>>& v2, DType dtype , bool requires_grad ) {
    ...
    for (size_t r = 0; r < rows; ++r) {
        for (size_t c = 0; c < cols; ++c) {
            write_scalar_at(out.impl->data->data.get(), idx++, out.impl->dtype, static_cast<double>(v2[r][c]));
        }
    }
    return out;
}
