...
Tensor sub_afterscalar(double scalar ,const Tensor& a ) {
    ...
    for (size_t i = 0; i < n; ++i) {
        double va = read_scalar_at(a.impl->storage->data.get(), i, a.impl->dtype);
        write_scalar_at(result.impl->storage->data.get(), i, result.impl->dtype, scalar - va );
    }
...
Tensor pow_scalar(const Tensor& a, double scalar) {
    if (!a.impl)
        throw std::runtime_error("pow_scalar: null tensor implementation");
    ...
    for (size_t i = 0; i < n; ++i) {
        double va = read_scalar_at(a.impl->storage->data.get(), i, a._dtype());
        write_scalar_at(result.impl->storage->data.get(), i, result._dtype(), std::pow(va, scalar));
    }

    if (a.requires_grad()) {
        result.impl->grad_fn = std::make_shared<GradPowScalar>(a, scalar);
    }
...
Tensor ln_(const Tensor& a_) {
    if (!a_.impl)
        throw std::runtime_error("ln_: null tensor implementation");
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        write_scalar_at(r_data, i, result.impl->dtype, std::log(val));
    }
...
Tensor exp_(const Tensor& a_) {
    if (!a_.impl)
        throw std::runtime_error("exp_: null tensor implementation");
    ...
    if (req) {
        result.impl->grad_fn = std::make_shared<GradExp>(a_);
    }
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, std::exp(val));
    }
...
Tensor sin_(const Tensor& a_) {
    if (!a_.impl)
        throw std::runtime_error("sin_: null tensor implementation");
    ...
    if (req) {
        result.impl->grad_fn = std::make_shared<GradSin>(a_);
    }
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, std::sin(val));
    }
...
Tensor asin_(const Tensor& a_) {
    if (!a_.impl)
        throw std::runtime_error("asin_: null tensor implementation");
    ...
    if (req) {
        result.impl->grad_fn = std::make_shared<GradASin>(a_);
    }
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, std::asin(val));
    }
...
Tensor cos_(const Tensor& a_) {
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, std::cos(val));
    }
...
Tensor acos_(const Tensor& a_) {
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, std::acos(val));
    }
...
Tensor cosh_(const Tensor& a_) {
    if (!a_.impl)
        throw std::runtime_error("cosh_: null tensor implementation");
    ...
    if (req) {
        result.impl->grad_fn = std::make_shared<GradCosH>(a_);
    }
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, std::cosh(val));
    }
...
Tensor tan_(const Tensor& a_) {
    ...
    if (req) {
        result.impl->grad_fn = std::make_shared<GradTan>(a_);
    }
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, std::tan(val));
    }
...
Tensor tanh_(const Tensor& a_) {
    if (!a_.impl)
        throw std::runtime_error("tanh_: null tensor implementation");
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, std::tanh(val));
    }
...
Tensor sigmoid_(const Tensor& a_) {
    if (!a_.impl)
        throw std::runtime_error("sigmoid_: null tensor implementation");
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, 1.0 / (1.0 + std::exp(-val)));
    }
...
Tensor Relu_(const Tensor& a_) {
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, val > 0.0 ? val : 0.0);
    }
...
Tensor softplus_(const Tensor& a_) {
    if (!a_.impl)
        throw std::runtime_error("softplus_: null tensor implementation");
    ...
    for (size_t i = 0; i < n; ++i) {
        double val = read_scalar_at(a_data, i, a_.impl->dtype);
        write_scalar_at(r_data, i, result.impl->dtype, std::log(1.0 + std::exp(val)));
    }
