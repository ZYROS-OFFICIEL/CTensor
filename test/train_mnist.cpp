...
#include <ctime>
#include <cmath>
#include <fstream> // Required to check for file existence
#include "core.h"
#include "neuralnet.h"
#include "check.h" // Include your checkpoint system
...
int main() {
    try {
        ...
        MLPNet model;
        // Get parameters once so we use the same list for Init, Optimizer, and Saving
        ...
        if (infile.good()) {
            infile.close();
            std::cout << "Found checkpoint: " << checkpoint_path << ". Loading weights..." << std::endl;
            checkpoints::load_weights(params, checkpoint_path);
        } else {
            infile.close();
            std::cout << "No checkpoint found. Initializing weights (Kaiming Uniform)..." << std::endl;
        ...
        SGD optim(params, 0.01);
        
        int BATCH_SIZE = 64;
        int EPOCHS = 5; // Increased epochs since we can now save progress
